...
func (r *postgresRepo) UserCreate(ctx context.Context, req *models.UserCreateReq) (*models.UserResponse, error) {
	...
	query := r.Db.Builder.Insert("users").Columns(
		"id,fullname, user_name, email, hashed_password, refresh_token",
	).Values(req.Id, req.FullName, req.UserName, req.Email, req.Password, req.RefreshToken).Suffix(
		"RETURNING id, fullname, user_name, email, hashed_password, refresh_token, created_at, updated_at")

	err := query.RunWith(r.Db.Db).Scan(
		&res.Id, &res.FullName, &res.UserName,
...
func (r *postgresRepo) UserGet(ctx context.Context, req *models.UserGetReq) (*models.UserResponse, error) {
	query := r.Db.Builder.Select("id,fullname, user_name, email, hashed_password, refresh_token, created_at, updated_at").
		From("users")
...
func (r *postgresRepo) UserFind(ctx context.Context, req *models.UserFindReq) (*models.UserFindResponse, error) {
	...
	query := r.Db.Builder.Select("id, fullname, user_name, created_at, updated_at").
	...
	for rows.Next() {
		...
		err := rows.Scan(
			&temp.Id, &temp.FullName, &temp.UserName,
			&CreatedAt, &UpdatedAt,
		)
		if err != nil {
			return res, HandleDatabaseError(err, r.Log, "(r *models.UserUserRepo) FindList()")
...
func (r *postgresRepo) UserUpdate(ctx context.Context, req *models.UserUpdateReq) (*models.UserResponse, error) {
	...
	mp["fullname"] = req.FullName
	mp["updated_at"] = time.Now()
	query := r.Db.Builder.Update("users").SetMap(mp).
		Where(squirrel.Eq{"id": req.Id}).
		Suffix("RETURNING id,fullname, user_name, email, hashed_password, refresh_token, created_at, updated_at")
